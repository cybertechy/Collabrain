...
function page()
{
	...
	useEffect(() =>
	{
		...
		{
			setToken(token);
			getInitialData(token).then((res) => { setIntialData(res); });
		});
	...
	useEffect(() =>
	{
		...
		sockCli.current.on('updateCollabData', (updateInfo) =>
		{
			...
			if (updateInfo?.collaborators?.length > 0) setTimeout(() => setConcurrencyStop(false), 1000);
		});
	...
	useEffect(() =>
	{
		...
		sockCli.current.io.on('reconnect_failed', () => setCollabaration(false));
	...
	const handleInputChange = (e) => { setContentMapName(e.target.value); };
