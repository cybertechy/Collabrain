...
const BadBehaviorStrikes = ({ strikes }) => {
  const renderSadFace = () => (
    <div style={{ marginRight: '10px', display: 'inline-block' }}>
      <SentimentVeryDissatisfiedIcon className="text-purple-500" fontSize="large" />
    </div>
  );
  return (
    <div>
      {Array.from({ length: strikes }).map(renderSadFace)}
...
<div>
<label className="relative group text-center"><input type="file" className="hidden" /><div className="group-hover:block relative w-24"><PersonIcon style={{ fontSize: 100 }} /><div className="absolute top-0 left-0 w-24 h-24 group-hover:bg-gray-300 opacity-0 group-hover:opacity-75 flex  items-center py-5 justify-center"><span className="text-black text-center font-bold">Upload</span></div></div></label>
</div>
...
const GeneralOverlay = ({setOpenModal, currentScreen, switchToGeneral, switchToProfile, switchToSound, switchToPrivacy, switchToNotifications, switchToAccessibility }) => {
...
<div className="w-2/4 h-5/6 shadow-lg bg-white border rounded-md flex overflow-auto ">
<button className='absolute top-24 pr-4 right-1/4 text-xl cursor-pointer' onClick={setOpenModal}>
<CloseIcon className = "text-black" fontSize="large" />
</button>
{/* LEFT SIDE */}
<OverlaySidebar currentScreen={currentScreen} switchToProfile={switchToProfile}  switchToGeneral={switchToGeneral} switchToSound={switchToSound}  switchToPrivacy={switchToPrivacy} switchToNotifications={switchToNotifications}switchToAccessibility={switchToAccessibility}/>
...
<button className="h-16 w-16 lg:w-32 lg:h-32 sm:h-auto sm:w-auto"><img src={'/assets/images/blackBG.png'} alt="image1" /></button>
<button className="h-16 w-16 lg:w-32 lg:h-32 sm:h-auto sm:w-auto"><img src={"/assets/images/purpleBG.png"} alt="image2" /></button>
</div>
<div className="mb-4">
...
//  SOUND SETTINGS PAGE
const SoundOverlay = ({ setOpenModal, currentScreen, switchToSound, switchToProfile, switchToGeneral, switchToPrivacy, switchToNotifications, switchToAccessibility}) => {
...
const PrivacyOverlay = ({ setOpenModal,currentScreen, switchToProfile, switchToPrivacy, switchToSound, switchToGeneral, switchToNotifications, switchToAccessibility,isToggled,handleToggle}) => {
  return (
<>
<div className="w-screen h-screen flex items-center justify-center ">
<div className="w-2/4 h-5/6 shadow-lg bg-white border rounded-md flex overflow-auto">
...
<div className="mb-4 flex justify-between">
<p className="p-2.5 sm:p-1 md:p-2 lg:p-2.5 text-2xl  text-black md:text-xl sm:text-sm lg:text-2xl   ">Export my data</p>
<button className="text-center text-xl md:text-lg sm:text-sm lg:text-xl inline-flex items-center border border-purple-500 text-purple-500 hover:bg-purple-500 hover:text-white px-7 py-3 md:px-5 md:py-2 sm:px-1 sm:py-1 lg:px-7 lg:py-3 ">Export</button>
</div>
<div className="mb-4 flex justify-between ">
...
const NotificationsOverlay = ({ setOpenModal,currentScreen, switchToNotifications, switchToProfile, switchToSound, switchToPrivacy, switchToGeneral, switchToAccessibility, isToggled, handleToggle }) => {
...
const AccessibilityOverlay = ({ currentScreen, switchToAccessibility, setOpenModal, switchToProfile, switchToSound, switchToPrivacy, switchToNotifications, switchToGeneral }) => {
  return (
<>
<div className="w-screen h-screen flex items-center justify-center ">
<div className="w-2/4 h-5/6 shadow-lg bg-white border rounded-md flex overflow-auto">
