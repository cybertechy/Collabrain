...
const SidebarItem = ({ href, icon: Icon, text, isSelected, isExpanded }) => {
    ...
    return (
        <Link href={href}>
            <div
                className={`flex w-64 items-center p-2 my-2 transition-colors duration-200 justify-start cursor-pointer hover:bg-gray-200`}
            >
                {Icon && (
                    <Icon fontSize="medium" className={`mr-2 ${itemClasses}`} />
                )}
                {isExpanded && (
                    <span
